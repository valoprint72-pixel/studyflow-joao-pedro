...
-- VERSÃO ULTRA SEGURA - VERIFICA TUDO ANTES DE CRIAR
-- =====================================================
...
-- Verificar e adicionar colunas faltantes nas tabelas existentes
DO $$
BEGIN
  -- Adicionar colunas faltantes em goals se não existirem
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'goals' AND column_name = 'points') THEN
    ALTER TABLE goals ADD COLUMN points INTEGER DEFAULT 0;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'goals' AND column_name = 'updated_at') THEN
    ALTER TABLE goals ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
  END IF;

  -- Adicionar colunas faltantes em habits se não existirem
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'habits' AND column_name = 'is_active') THEN
  ...
  -- Adicionar colunas faltantes em achievements se não existirem
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'achievements' AND column_name = 'goals_required') THEN
    ALTER TABLE achievements ADD COLUMN goals_required INTEGER DEFAULT 0;
  END IF;
  ...
  -- Adicionar colunas faltantes em pomodoro_sessions se não existirem
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pomodoro_sessions' AND column_name = 'status') THEN
    ALTER TABLE pomodoro_sessions ADD COLUMN status VARCHAR(20) DEFAULT 'active';
  END IF;

END $$;

-- =====================================================
-- ÍNDICES PARA PERFORMANCE (VERSÃO SEGURA)
-- =====================================================

-- Índices básicos que sempre existem
...
-- Índices condicionais (só criam se as colunas existirem)
DO $$
BEGIN
  -- Índices para tasks
  ...
  -- Índices para habits
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'habits' AND column_name = 'is_active') THEN
    CREATE INDEX IF NOT EXISTS idx_habits_active ON habits(is_active);
  END IF;

  -- Índices para notifications
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'scheduled_for') THEN
...
BEGIN
  ...
  -- Pontos de metas completadas (verificar se a coluna status existe)
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'goals' AND column_name = 'status') THEN
...
BEGIN
  ...
  -- Atualizar streak no hábito (verificar se a coluna updated_at existe)
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'habits' AND column_name = 'updated_at') THEN
    UPDATE habits 
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'goals_progress', (
      ...
      AND (NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'goals' AND column_name = 'status') 
           OR g.status = 'active')
    ),
    'tasks_completed', (
      SELECT COUNT(*) 
